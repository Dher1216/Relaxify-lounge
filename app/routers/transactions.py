...
def list_transactions(
    ...
    status: str = None, q: str = None, page: int = 1, sales_only: str = None,
):
    user = get_current_user(request, db)
    if not user:
        return RedirectResponse("/login", status_code=303)
    ...
    if sales_only == "1":
        # "Sales" here is detected the same way the dashboard detects it: any
        # transaction with a line crediting Service Revenue or one of its
        # sub-accounts (Deluxe/King/Add-ons) - not by transaction type, so a
        # loan receipt or other non-sale Receipt never shows up in this view.
        ...
        if service_revenue:
            ...
            sale_txn_ids = (
                db.query(TransactionLine.transaction_id)
                .filter(TransactionLine.account_id.in_(revenue_account_ids))
                .filter(TransactionLine.credit > 0)
                .distinct()
            )
            query = query.filter(Transaction.id.in_(sale_txn_ids))
        else:
            query = query.filter(Transaction.id == None)  # noqa: E711 - no Service Revenue account configured
    ...
    # Debit total always equals credit total for a saved (balanced) transaction, so
    # there's just one real "amount" per transaction worth showing in a list view.
    txn_amounts = {t.id: sum((l.debit for l in t.lines), Decimal("0.00")) for t in txns}
    ...
    return templates.TemplateResponse("transactions_list.html", {
        ...
        "sales_only": sales_only, "page_title": "Today's Sales" if sales_only == "1" else "All Transactions",
