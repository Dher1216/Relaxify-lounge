...
# Accounts flagged as genuine cash/bank/e-wallet ASSETS for dashboard cash-flow math.
# Deliberately an explicit list, not a name-text search - a text search for "cash" would
# also match things like a "LP- Gcash" loan payable account (GCash the e-wallet, not
# cash), silently corrupting the Receipts/Disbursements totals. Add new codes here (or
# via the Account Codes screen, once we add a checkbox there) whenever a genuine new
# cash-equivalent account is created - e.g. if GCash is ever added as an actual asset
# account for tracking e-wallet balance, not just as a name coincidence.
...
def run():
    ...
    try:
        ...
        # --- Flag genuine cash accounts (runs every startup - safe, only ever sets
        # is_cash_account=True on the known-correct codes, never touches anything else) ---
        for acc in db.query(Account).filter(Account.code.in_(CASH_ACCOUNT_CODES)).all():
            if not acc.is_cash_account:
                acc.is_cash_account = True
        db.flush()
